...
local function defaults()
    return {
        ...
        flagsEnabled = false,
        lineEnabled = false,
        lineOrigin = "Bottom", -- Bottom, Top, Center, Mouse
        tracerColor = "Blue",
        teamCheckEnabled = false,
        espGunHighlight = true,
        espShowWeapon = false,
        espShowFlags = false,
...
local function getPlayerRole(player)
    ...
    if hasTool("Knife") or hasTool("CKnife") then
        return "Murderer"
    end
    if hasTool("Gun") or hasTool("Revolver") or hasTool("CGun") then
        return "Sheriff"
    end

    local mm2 = workspace:FindFirstChild("MurderMystery2")
    if mm2 then
        local murdererVal = mm2:FindFirstChild("Murderer")
        if murdererVal and murdererVal:IsA("ObjectValue") and murdererVal.Value == player then
            return "Murderer"
        end
        local sheriffVal = mm2:FindFirstChild("Sheriff")
        if sheriffVal and sheriffVal:IsA("ObjectValue") and sheriffVal.Value == player then
            return "Sheriff"
        end
    end
...
-- ============================================================
-- COMPLETE ESP SYSTEM (with all customization)
-- ============================================================
local espData = {} -- per player: {highlight, billboard, box, name, role, health, dist, weapon, flags, line}
...
local function createESP(player)
    ...
    if espData[player] then
        ...
        if espData[player].line then espData[player].line:Remove() end
        espData[player] = nil
    end
    ...
    roleLbl.Size = UDim2.new(1, 0, 0.18, 0)
    ...
    healthBg.Position = UDim2.new(0.1, 0, 0.92, 0)
    ...
    distLbl.Size = UDim2.new(1, 0, 0.15, 0)
    distLbl.Position = UDim2.new(0, 0, -0.25, 0)
    ...
    weaponLbl.Size = UDim2.new(1, 0, 0.15, 0)
    weaponLbl.Position = UDim2.new(0, 0, 0.6, 0)
    weaponLbl.BackgroundTransparency = 1
    weaponLbl.Text = ""
    weaponLbl.TextColor3 = Color3.fromRGB(200, 200, 200)
    weaponLbl.Font = Enum.Font.Gotham
    weaponLbl.TextSize = 10
    weaponLbl.Visible = false
    weaponLbl.Parent = frame

    -- Flags
    local flagLbl = Instance.new("TextLabel")
    flagLbl.Size = UDim2.new(1, 0, 0.15, 0)
    flagLbl.Position = UDim2.new(0, 0, 0.75, 0)
    flagLbl.BackgroundTransparency = 1
    flagLbl.Text = ""
    flagLbl.TextColor3 = Color3.fromRGB(255, 255, 100)
    flagLbl.Font = Enum.Font.Gotham
    flagLbl.TextSize = 10
    flagLbl.Visible = false
    flagLbl.Parent = frame

    -- Line tracer (Drawing)
    local line = nil
    if Yuno.lineEnabled then
...
local function updateESP()
    for player, data in pairs(espData) do
        ...
        -- Highlight
        data.highlight.Enabled = show and Yuno.highlightEnabled
        data.highlight.FillColor = color

        -- Billboard visibility
        data.billboard.Enabled = show
        if show then
            -- Update role, border, colors
            data.role.Text = role:upper()
            data.role.TextColor3 = color
            data.frame.BorderColor3 = color

            -- Health
            if Yuno.healthEnabled then
                local hum = char:FindFirstChildOfClass("Humanoid")
                if hum then
                    local hp = hum.Health / hum.MaxHealth
                    data.health.Size = UDim2.new(hp, 0, 1, 0)
                    data.health.BackgroundColor3 = Color3.fromRGB(255 - 255 * hp, 255 * hp, 0)
            ...
            -- Name
            data.name.Visible = Yuno.nameEnabled
            -- Role
            data.role.Visible = Yuno.roleEnabled

            -- Weapon
            if Yuno.weaponEnabled then
                local weapon = char:FindFirstChildOfClass("Tool")
                if weapon then
                    data.weapon.Text = weapon.Name
            ...
            -- Flags
            if Yuno.flagsEnabled then
                local hum = char:FindFirstChildOfClass("Humanoid")
                if hum then
                    local flags = {}
                    if hum.MoveDirection.Magnitude > 0 then table.insert(flags, "Walking") end
                    if hum:GetState() == Enum.HumanoidStateType.Jumping then table.insert(flags, "Jumping") end
                    if hum:GetState() == Enum.HumanoidStateType.Swimming then table.insert(flags, "Swimming") end
                    data.flags.Text = table.concat(flags, " ")
                    data.flags.Visible = #flags > 0
                else
            ...
            -- Line tracer
            if Yuno.lineEnabled and data.line then
                local head = char:FindFirstChild("Head")
                if head then
                    local pos, onScreen = Camera:WorldToViewportPoint(head.Position)
                    if onScreen then
                        local viewport = Camera.ViewportSize
                        local origin = Yuno.lineOrigin
                        local from
...
-- Gun ESP (highlight dropped guns)
local gunEspObjects = {}
local function updateGunESP()
    if not Yuno.espGunHighlight then
...
local function getAimbotTarget()
    ...
    local targetRole = Yuno.aimbotTarget
    local targetName = Yuno.aimbotPlayerName
    ...
    for _, player in ipairs(Players:GetPlayers()) do
        ...
        local match = false
        if targetRole == "Murderer" and role == "Murderer" then
            match = true
        elseif targetRole == "Sheriff" and role == "Sheriff" then
            match = true
        elseif targetRole == "Innocent" and role == "Innocent" then
            match = true
        elseif targetRole == "Player" and (player.Name:lower():find(targetName:lower()) or player.DisplayName:lower():find(targetName:lower())) then
            match = true
        end
...
local function autoShootLoop()
    while RunService.RenderStepped:Wait() do
        ...
        if gun and gun:FindFirstChild("KnifeLocal") then
            local remote = gun.KnifeLocal.CreateBeam.RemoteFunction
            if remote then
                remote:InvokeServer(1, tHead.Position, "AH2")
            end
        end
        task.wait(0.1)
    end
...
local function autoKillLoop()
    while true do
        if not (Yuno.autoKillMurdererEnabled or Yuno.autoKillSheriffEnabled) then
            task.wait(0.5)
            continue
        end
        local char = LocalPlayer.Character
        ...
        for _, player in ipairs(Players:GetPlayers()) do
            ...
            if myRole == "Murderer" and Yuno.autoKillMurdererEnabled and role ~= "Murderer" then
                shouldKill = true
            elseif myRole == "Sheriff" and Yuno.autoKillSheriffEnabled and role == "Murderer" then
                shouldKill = true
            end
            if not shouldKill then continue end
...
-- ============================================================
-- NOCLIP, INFINITE JUMP, BHOP, SPEED, JUMP POWER, SPINBOT, SWIM WALK
-- ============================================================
RunService.Stepped:Connect(function()
...
RunService.Heartbeat:Connect(function()
    if Yuno.touchFlingEnabled then
        local myRoot = getHumanoidRootPart()
...
-- Fling Murderer/Sheriff buttons
...
-- ============================================================
-- WEAPON SPAWNER / DUPE / REPLACER / SKIN CHANGER
-- ============================================================
...
-- TROLLING ANIMATION
-- ============================================================
local trollTrack = nil
RunService.RenderStepped:Connect(function()
    if Yuno.trollAnimEnabled then
        local hum = getHumanoid()
        if hum then
            local animator = hum:FindFirstChildOfClass("Animator")
            if animator then
                if not trollTrack then
                    local anim = Instance.new("Animation")
                    anim.AnimationId = "rbxassetid://507770677"
                    trollTrack = animator:LoadAnimation(anim)
                    trollTrack.Looped = true
                    trollTrack:Play()
                end
                trollTrack:AdjustSpeed(Yuno.trollIntensity * 0.5)
            end
        end
    else
        if trollTrack then
...
RunService.RenderStepped:Connect(function()
    if Yuno.fpsCounterEnabled then
...
ESPTab:Toggle({ Flag = "ESP_Enabled", Title = "Enable ESP", Default = Yuno.espEnabled, Callback = function(v) Yuno.espEnabled = v end })
ESPTab:Toggle({ Flag = "ESP_Highlight", Title = "Highlight (Chams)", Default = Yuno.highlightEnabled, Callback = function(v) Yuno.highlightEnabled = v end })
ESPTab:Toggle({ Flag = "ESP_Box", Title = "Box ESP", Default = Yuno.boxEnabled, Callback = function(v) Yuno.boxEnabled = v end })
ESPTab:Dropdown({ Flag = "ESP_BoxType", Title = "Box Type", Values = { "2D", "Corner" }, Value = Yuno.boxType, Callback = function(v) Yuno.boxType = v end })
ESPTab:Toggle({ Flag = "ESP_BoxGlow", Title = "Box Glow", Default = Yuno.boxGlowEnabled, Callback = function(v) Yuno.boxGlowEnabled = v end })
ESPTab:Toggle({ Flag = "ESP_BoxFilled", Title = "Filled Box", Default = Yuno.boxFilledEnabled, Callback = function(v) Yuno.boxFilledEnabled = v end })
ESPTab:Toggle({ Flag = "ESP_Name", Title = "Show Names", Default = Yuno.nameEnabled, Callback = function(v) Yuno.nameEnabled = v end })
ESPTab:Toggle({ Flag = "ESP_Role", Title = "Show Role", Default = Yuno.roleEnabled, Callback = function(v) Yuno.roleEnabled = v end })
ESPTab:Toggle({ Flag = "ESP_Health", Title = "Show Health", Default = Yuno.healthEnabled, Callback = function(v) Yuno.healthEnabled = v end })
ESPTab:Toggle({ Flag = "ESP_Distance", Title = "Show Distance", Default = Yuno.distanceEnabled, Callback = function(v) Yuno.distanceEnabled = v end })
ESPTab:Toggle({ Flag = "ESP_Weapon", Title = "Show Weapon", Default = Yuno.weaponEnabled, Callback = function(v) Yuno.weaponEnabled = v end })
ESPTab:Toggle({ Flag = "ESP_Flags", Title = "Show Movement Flags", Default = Yuno.flagsEnabled, Callback = function(v) Yuno.flagsEnabled = v end })
ESPTab:Toggle({ Flag = "ESP_Line", Title = "Line Tracers", Default = Yuno.lineEnabled, Callback = function(v) Yuno.lineEnabled = v end })
ESPTab:Dropdown({ Flag = "ESP_LineOrigin", Title = "Tracer Origin", Values = { "Bottom", "Top", "Center", "Mouse" }, Value = Yuno.lineOrigin, Callback = function(v) Yuno.lineOrigin = v end })
ESPTab:Dropdown({ Flag = "ESP_Filter", Title = "Player Filter", Values = { {Title = "All", Icon="users"}, {Title = "Murderer", Icon="skull"}, {Title = "Sheriff", Icon="shield"}, {Title = "Murderer/Sheriff", Icon="target"} }, Value = Yuno.espFilter, Callback = function(selected) Yuno.espFilter = selected.Title end })
ESPTab:Toggle({ Flag = "ESP_TeamCheck", Title = "Ignore Teammates", Default = Yuno.teamCheckEnabled, Callback = function(v) Yuno.teamCheckEnabled = v end })
ESPTab:Toggle({ Flag = "ESP_Gun", Title = "Gun ESP (highlight dropped gun)", Default = Yuno.espGunHighlight, Callback = function(v) Yuno.espGunHighlight = v end })
...
VisualTab:Toggle({ Flag = "Xray", Title = "Xray (see through walls)", Default = Yuno.xrayEnabled, Callback = function(v) Yuno.xrayEnabled = v; if v then enableXray() else disableXray() end end })
VisualTab:Toggle({ Flag = "FullBright", Title = "Full Bright", Default = Yuno.fullBrightEnabled, Callback = function(v) Yuno.fullBrightEnabled = v; setFullBright(v) end })
VisualTab:Dropdown({ Flag = "Skybox", Title = "Skybox Style", Values = { "Original", "Sunset", "Galaxy", "Night" }, Value = Yuno.skybox, Callback = function(v) Yuno.skybox = v; setSkybox(v) end })
VisualTab:Section({ Title = "Hitboxes", TextSize = 18, FontWeight = Enum.FontWeight.SemiBold })
VisualTab:Toggle({ Flag = "HitboxScale", Title = "Scale Hitboxes", Default = Yuno.hitboxScaleEnabled, Callback = function(v) Yuno.hitboxScaleEnabled = v end })
VisualTab:Slider({ Flag = "HitboxScaleValue", Title = "Hitbox Scale", Step = 0.5, Value = { Min = 1, Max = 10, Default = Yuno.hitboxScale }, Callback = function(v) Yuno.hitboxScale = v end })
...
CombatTab:Toggle({ Flag = "Aimbot", Title = "Enable Aimbot", Default = Yuno.aimbotEnabled, Callback = function(v) Yuno.aimbotEnabled = v end })
CombatTab:Dropdown({ Flag = "AimbotTarget", Title = "Target Role / Player", Values = { {Title = "Murderer", Icon="skull"}, {Title = "Sheriff", Icon="shield"}, {Title = "Innocent", Icon="user"}, {Title = "Player (by name)", Icon="user"} }, Value = Yuno.aimbotTarget, Callback = function(selected) Yuno.aimbotTarget = selected.Title end })
CombatTab:Input({ Flag = "AimbotPlayerName", Title = "Player Name (if target is Player)", Placeholder = "Enter name", Value = Yuno.aimbotPlayerName, Callback = function(v) Yuno.aimbotPlayerName = v end })
CombatTab:Slider({ Flag = "AimbotSmooth", Title = "Smoothness", Step = 1, Value = { Min = 1, Max = 20, Default = Yuno.aimbotSmooth }, Callback = function(v) Yuno.aimbotSmooth = v end })
CombatTab:Slider({ Flag = "AimbotFOV", Title = "FOV (degrees)", Step = 5, Value = { Min = 30, Max = 180, Default = Yuno.aimbotFOV }, Callback = function(v) Yuno.aimbotFOV = v end })

CombatTab:Section({ Title = "Auto Shoot (Sheriff)", TextSize = 18, FontWeight = Enum.FontWeight.SemiBold })
CombatTab:Toggle({ Flag = "AutoShoot", Title = "Auto Shoot Murderer (headshot)", Default = Yuno.autoShootEnabled, Callback = function(v) Yuno.autoShootEnabled = v end })

CombatTab:Section({ Title = "Auto Kill", TextSize = 18, FontWeight = Enum.FontWeight.SemiBold })
CombatTab:Toggle({ Flag = "AutoKillMurderer", Title = "Auto Kill (as Murderer)", Default = Yuno.autoKillMurdererEnabled, Callback = function(v) Yuno.autoKillMurdererEnabled = v end })
CombatTab:Toggle({ Flag = "AutoKillSheriff", Title = "Auto Kill Murderer (as Sheriff)", Default = Yuno.autoKillSheriffEnabled, Callback = function(v) Yuno.autoKillSheriffEnabled = v end })

CombatTab:Section({ Title = "Kill All (Murderer)", TextSize = 18, FontWeight = Enum.FontWeight.SemiBold })
CombatTab:Toggle({ Flag = "KillAll", Title = "Enable Kill All", Default = Yuno.killAllEnabled, Callback = function(v) Yuno.killAllEnabled = v; if v then startKillAll() else stopKillAll() end end })
CombatTab:Slider({ Flag = "KillAll_Delay", Title = "Attack Delay", Step = 0.1, Value = { Min = 0.1, Max = 2, Default = Yuno.killAllDelay }, Callback = function(v) Yuno.killAllDelay = v end })

CombatTab:Section({ Title = "Kill Aura", TextSize = 18, FontWeight = Enum.FontWeight.SemiBold })
CombatTab:Toggle({ Flag = "KillAura", Title = "Enable Kill Aura", Default = Yuno.killAuraEnabled, Callback = function(v) Yuno.killAuraEnabled = v end })
CombatTab:Slider({ Flag = "KillAuraRange", Title = "Range", Step = 1, Value = { Min = 5, Max = 100, Default = Yuno.killAuraRange }, Callback = function(v) Yuno.killAuraRange = v end })
CombatTab:Slider({ Flag = "KillAuraDamage", Title = "Damage per tick", Step = 0.5, Value = { Min = 1, Max = 10, Default = Yuno.killAuraDamage }, Callback = function(v) Yuno.killAuraDamage = v end })

CombatTab:Section({ Title = "Weapon Mods", TextSize = 18, FontWeight = Enum.FontWeight.SemiBold })
CombatTab:Toggle({ Flag = "InfiniteAmmo", Title = "Infinite Ammo", Default = Yuno.infiniteAmmoEnabled, Callback = function(v) Yuno.infiniteAmmoEnabled = v end })
CombatTab:Toggle({ Flag = "InstantReload", Title = "Instant Reload", Default = Yuno.instantReloadEnabled, Callback = function(v) Yuno.instantReloadEnabled = v end })
CombatTab:Button({ Title = "Instant Round Win", Icon = "trophy", Callback = function() instantWin() end })
...
FarmTab:Toggle({ Flag = "CoinFarm", Title = "Coin Autofarm", Default = Yuno.coinFarmEnabled, Callback = function(v) Yuno.coinFarmEnabled = v end })
FarmTab:Toggle({ Flag = "CandyFarm", Title = "Candy Autofarm", Default = Yuno.candyFarmEnabled, Callback = function(v) Yuno.candyFarmEnabled = v end })
FarmTab:Slider({ Flag = "FarmSpeed", Title = "Movement Speed", Step = 1, Value = { Min = 5, Max = 50, Default = Yuno.farmSpeed }, Callback = function(v) Yuno.farmSpeed = v end })
FarmTab:Toggle({ Flag = "AutoReset", Title = "Auto Reset on Bag Full", Default = Yuno.autoResetOnBagFull, Callback = function(v) Yuno.autoResetOnBagFull = v end })
FarmTab:Toggle({ Flag = "AutoFlingMurderer", Title = "Auto Fling Murderer on Full", Default = Yuno.autoFlingMurdererOnFull, Callback = function(v) Yuno.autoFlingMurdererOnFull = v end })
...
MoveTab:Toggle({ Flag = "Fly", Title = "Fly (WASD + Space/Shift)", Default = Yuno.flyEnabled, Callback = function(v) toggleFly(v) end })
MoveTab:Slider({ Flag = "FlySpeed", Title = "Fly Speed", Step = 1, Value = { Min = 10, Max = 200, Default = Yuno.flySpeed }, Callback = function(v) Yuno.flySpeed = v end })
MoveTab:Toggle({ Flag = "Noclip", Title = "Noclip", Default = Yuno.noclipEnabled, Callback = function(v) Yuno.noclipEnabled = v end })
MoveTab:Toggle({ Flag = "InfiniteJump", Title = "Infinite Jump", Default = Yuno.infiniteJumpEnabled, Callback = function(v) Yuno.infiniteJumpEnabled = v end })
MoveTab:Toggle({ Flag = "BunnyHop", Title = "Bunny Hop", Default = Yuno.bhopEnabled, Callback = function(v) Yuno.bhopEnabled = v end })
MoveTab:Slider({ Flag = "BhopSpeed", Title = "Bhop Speed", Step = 1, Value = { Min = 10, Max = 80, Default = Yuno.bhopSpeed }, Callback = function(v) Yuno.bhopSpeed = v end })
MoveTab:Toggle({ Flag = "SpeedHack", Title = "Speed Hack", Default = Yuno.speedHackEnabled, Callback = function(v) Yuno.speedHackEnabled = v end })
MoveTab:Slider({ Flag = "SpeedValue", Title = "Speed Value", Step = 1, Value = { Min = 1, Max = 200, Default = Yuno.speedValue }, Callback = function(v) Yuno.speedValue = v end })
MoveTab:Toggle({ Flag = "SpeedGlitch", Title = "Speed Glitch (super sprint)", Default = Yuno.speedGlitchEnabled, Callback = function(v) Yuno.speedGlitchEnabled = v end })
MoveTab:Toggle({ Flag = "JumpPower", Title = "Custom Jump Power", Default = Yuno.jumpPowerEnabled, Callback = function(v) Yuno.jumpPowerEnabled = v end })
MoveTab:Slider({ Flag = "JumpPowerValue", Title = "Jump Power", Step = 1, Value = { Min = 10, Max = 300, Default = Yuno.jumpPowerValue }, Callback = function(v) Yuno.jumpPowerValue = v end })
MoveTab:Toggle({ Flag = "Spinbot", Title = "Spinbot", Default = Yuno.spinbotEnabled, Callback = function(v) Yuno.spinbotEnabled = v end })
MoveTab:Toggle({ Flag = "SwimWalk", Title = "Swim Walk (fake swimming)", Default = Yuno.swimWalkEnabled, Callback = function(v) Yuno.swimWalkEnabled = v end })
...
TeleportTab:Toggle({ Flag = "TPGun", Title = "Teleport to Gun", Default = Yuno.tpGunEnabled, Callback = function(v) Yuno.tpGunEnabled = v end })
TeleportTab:Button({ Title = "Teleport to Gun Now", Icon = "zap", Callback = function() tpToGun() end })
TeleportTab:Toggle({ Flag = "GrabGun", Title = "Auto Grab Gun", Default = Yuno.grabGunEnabled, Callback = function(v) Yuno.grabGunEnabled = v; if v then task.spawn(autoGrabLoop) end end })
TeleportTab:Button({ Title = "Grab Gun Now", Icon = "hand", Callback = function() grabGun() end })
TeleportTab:Button({ Title = "Rejoin Server", Icon = "refresh-cw", Callback = function() rejoinServer() end })
...
WeaponsTab:Input({ Flag = "SpawnName", Title = "Weapon Name", Placeholder = "e.g., Raygun", Value = Yuno.spawnWeaponName, Callback = function(v) Yuno.spawnWeaponName = v end })
WeaponsTab:Button({ Title = "Spawn Weapon", Icon = "sparkles", Callback = function() spawnWeapon(Yuno.spawnWeaponName) end })

WeaponsTab:Section({ Title = "Duplicate Weapon", TextSize = 18, FontWeight = Enum.FontWeight.SemiBold })
WeaponsTab:Input({ Flag = "DupeName", Title = "Weapon Name", Placeholder = "e.g., Lightbringer", Value = Yuno.dupeWeaponName, Callback = function(v) Yuno.dupeWeaponName = v end })
WeaponsTab:Input({ Flag = "DupeAmount", Title = "Dupe Amount", Value = tostring(Yuno.dupeAmount), Callback = function(v) Yuno.dupeAmount = tonumber(v) or 1 end })
WeaponsTab:Button({ Title = "Duplicate Weapon", Icon = "copy", Callback = function() for i=1, Yuno.dupeAmount do duplicateWeapon(Yuno.dupeWeaponName) end end })
WeaponsTab:Button({ Title = "Duplicate Entire Inventory", Icon = "package", Callback = function() duplicateInventory() end })

WeaponsTab:Section({ Title = "Visual Weapon Replacer", TextSize = 18, FontWeight = Enum.FontWeight.SemiBold })
WeaponsTab:Input({ Flag = "FromWeapon", Title = "From Weapon", Placeholder = "e.g., Blossom", Value = Yuno.fromWeapon, Callback = function(v) Yuno.fromWeapon = v end })
WeaponsTab:Input({ Flag = "ToWeapon", Title = "To Weapon", Placeholder = "e.g., Chroma", Value = Yuno.toWeapon, Callback = function(v) Yuno.toWeapon = v end })
WeaponsTab:Button({ Title = "Replace Visual", Icon = "wand-2", Callback = function() replaceWeapon(Yuno.fromWeapon, Yuno.toWeapon) end })

WeaponsTab:Section({ Title = "Skin Changer", TextSize = 18, FontWeight = Enum.FontWeight.SemiBold })
WeaponsTab:Toggle({ Flag = "SkinChanger", Title = "Enable Skin Changer", Default = Yuno.skinChangerEnabled, Callback = function(v) Yuno.skinChangerEnabled = v end })
WeaponsTab:Input({ Flag = "KnifeMesh", Title = "Knife Texture ID", Placeholder = "rbxassetid://...", Value = Yuno.selectedKnifeMesh, Callback = function(v) Yuno.selectedKnifeMesh = v end })
WeaponsTab:Input({ Flag = "GunMesh", Title = "Gun Texture ID", Placeholder = "rbxassetid://...", Value = Yuno.selectedGunMesh, Callback = function(v) Yuno.selectedGunMesh = v end })
...
FlingTab:Slider({ Flag = "FlingPower", Title = "Fling Power", Step = 5, Value = { Min = 20, Max = 300, Default = Yuno.flingPower }, Callback = function(v) Yuno.flingPower = v end })
FlingTab:Input({ Flag = "FlingTargetName", Title = "Target Player Name", Placeholder = "Enter name to fling", Value = Yuno.flingTargetName, Callback = function(v) Yuno.flingTargetName = v end })
FlingTab:Button({ Title = "Fling Target", Icon = "zap", Callback = function()
    if Yuno.flingTargetName ~= "" then
        if flingTargetByName(Yuno.flingTargetName) then
            WindUI:Notify({Title="Fling", Content="Flinged "..Yuno.flingTargetName, Icon="check", Duration=3})
        else
            WindUI:Notify({Title="Fling", Content="Player not found", Icon="x", Duration=3})
        end
    end
end })
FlingTab:Toggle({ Flag = "FlingAll", Title = "Fling All Players", Default = Yuno.flingAllEnabled, Callback = function(v) Yuno.flingAllEnabled = v; if v then flingAll() end end })
FlingTab:Toggle({ Flag = "TouchFling", Title = "Touch Fling (on contact)", Default = Yuno.touchFlingEnabled, Callback = function(v) Yuno.touchFlingEnabled = v end })
FlingTab:Button({ Title = "Fling Murderer", Icon = "skull", Callback = function() flingMurderer() end })
FlingTab:Button({ Title = "Fling Sheriff", Icon = "shield", Callback = function() flingSheriff() end })
...
MiscTab:Toggle({ Flag = "AntiAFK", Title = "Anti-AFK", Default = Yuno.antiAFKEnabled, Callback = function(v) Yuno.antiAFKEnabled = v; if v then LocalPlayer.Idled:Connect(function() VirtualUser:Button2Down(Vector2.new(0,0), Camera.CFrame); task.wait(1); VirtualUser:Button2Up(Vector2.new(0,0), Camera.CFrame) end) end end })
MiscTab:Toggle({ Flag = "AutoGreet", Title = "Auto Greet (say in chat once)", Default = Yuno.autoGreetEnabled, Callback = function(v) Yuno.autoGreetEnabled = v end })
MiscTab:Toggle({ Flag = "BulletTracers", Title = "Bullet Tracers", Default = Yuno.bulletTracersEnabled, Callback = function(v) Yuno.bulletTracersEnabled = v end })
MiscTab:Dropdown({ Flag = "TracerColor", Title = "Tracer Color", Values = { "Blue", "Red", "Green", "Rainbow" }, Value = Yuno.tracerColor, Callback = function(v) Yuno.tracerColor = v end })
MiscTab:Toggle({ Flag = "FpsCounter", Title = "Show FPS Counter", Default = Yuno.fpsCounterEnabled, Callback = function(v) Yuno.fpsCounterEnabled = v; if v then createFPSCounter() elseif fpsLabel then fpsLabel:Destroy(); fpsLabel = nil end end })
MiscTab:Toggle({ Flag = "TrollAnim", Title = "Troll Animation (hand jerk)", Default = Yuno.trollAnimEnabled, Callback = function(v) Yuno.trollAnimEnabled = v end })
MiscTab:Slider({ Flag = "TrollIntensity", Title = "Troll Intensity", Step = 1, Value = { Min = 1, Max = 50, Default = Yuno.trollIntensity }, Callback = function(v) Yuno.trollIntensity = v end })
MiscTab:Toggle({ Flag = "TradeScam", Title = "Trade Scam (visual)", Default = Yuno.tradeScamEnabled, Callback = function(v) Yuno.tradeScamEnabled = v end })
MiscTab:Button({ Title = "Activate Trade Scam", Icon = "alert-triangle", Callback = function() activateTradeScam() end })
MiscTab:Toggle({ Flag = "ServerLagger", Title = "Server Lagger (use with caution)", Default = Yuno.serverLaggerEnabled, Callback = function(v) Yuno.serverLaggerEnabled = v; if v then serverLagger() end end })
...
SettingsTab:Keybind({ Flag = "MenuKeybind", Title = "Toggle GUI Key", Value = Yuno.menuKeybind, Callback = function(key) Yuno.menuKeybind = key; Window:SetToggleKey(Enum.KeyCode[key]) end })
SettingsTab:Section({ Title = "Config Management", TextSize = 18, FontWeight = Enum.FontWeight.SemiBold })
local configInput = SettingsTab:Input({ Flag = "ConfigName", Title = "Config Name", Value = Yuno.configName, Callback = function(v) Yuno.configName = v end })
local configDropdown = SettingsTab:Dropdown({
    ...
    Callback = function(selected) Yuno.configName = selected.Title; configInput:Set(selected.Title) end
})
SettingsTab:Button({ Title = "Save Config", Icon = "save", Callback = function() saveConfig(Yuno.configName); WindUI:Notify({Title="Config Saved", Content="Saved as "..Yuno.configName, Icon="check", Duration=3}) end })
SettingsTab:Button({ Title = "Load Config", Icon = "upload", Callback = function() if loadConfig(Yuno.configName) then WindUI:Notify({Title="Config Loaded", Content="Loaded "..Yuno.configName, Icon="check", Duration=3}) else WindUI:Notify({Title="Error", Content="Config not found", Icon="x", Duration=3}) end end })
SettingsTab:Button({ Title = "Refresh Config List", Icon = "refresh-cw", Callback = function() configDropdown:Refresh() end })
...
InfoTab:Section({ Title = "All-in-one MM2 Hub with all features from Benjo, Onyyx, Dabl Gee, Fan Hub, Advanced ESP, Item Spawner, and more.", TextSize = 16, TextTransparency = 0.3 })
InfoTab:Section({ Title = "Features: ESP (Highlight, Box, Name, Role, Health, Distance, Tracers, Weapon, Flags, Gun) | Xray | Full Bright | Skybox | Hitboxes | Aimbot (with target selection) | Auto Shoot (headshot) | Auto Kill (Murderer & Sheriff) | Kill All | Kill Aura | Infinite Ammo | Instant Reload | Auto Farm (Coins & Candy) | Fly (with mobile joystick) | Noclip | Infinite Jump | Bunny Hop | Speed Hack | Speed Glitch | Jump Power | Spinbot | Swim Walk | Teleport to Gun | Grab Gun | Weapon Spawn/Dupe/Replacer | Skin Changer | Fling (any player, all, touch, murderer, sheriff) | Trade Scam | Server Lagger | Anti-AFK | Bullet Tracers | Troll Animation | Auto Greet | Rejoin | FPS Counter | Config System", TextSize = 14, TextTransparency = 0.4 })
...
if Yuno.fpsCounterEnabled then createFPSCounter() end
if Yuno.killAllEnabled then startKillAll() end
if Yuno.flyEnabled then toggleFly(true) end
if Yuno.grabGunEnabled then task.spawn(autoGrabLoop) end
...
print("Yuno Hub loaded with all features including full ESP customization and fling any player!")