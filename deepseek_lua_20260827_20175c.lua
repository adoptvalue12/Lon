--[[
    ...
    All-in-One Hub with WindUI
    Includes all features from: Benjo, Onyyx, Dabl Gee, Fan Hub, and more.
    Enhanced with: Fling any player, FPS counter, Aimbot target selection, improved ESP, Rejoin.
]]
...
local function defaults()
    return {
        ...
        -- Combat (Aimbot, Auto Shoot, Kill All, Kill Aura, Weapon Mods)
        aimbotEnabled = false,
        aimbotTarget = "Murderer", -- Murderer, Sheriff, Innocent, Player
        aimbotPlayerName = "",
        aimbotSmooth = 5,
        aimbotFOV = 90,
...
-- ============================================================
-- ESP SYSTEM (BillboardGui + Highlight + Lines)
-- ============================================================
...
local function createEspForPlayer(player)
    ...
    -- Frame for box background (always present)
    ...
    weaponLabel.Size = UDim2.new(1, 0, 0.15, 0)
    weaponLabel.Position = UDim2.new(0, 0, 0.65, 0)
    ...
    flagLabel.Size = UDim2.new(1, 0, 0.15, 0)
    flagLabel.Position = UDim2.new(0, 0, 0.8, 0)
    flagLabel.BackgroundTransparency = 1
    flagLabel.Text = ""
    flagLabel.TextColor3 = Color3.fromRGB(255, 255, 100)
...
local function updateEspForPlayer(player)
    ...
    if show then
        ...
        -- Weapon
        if Yuno.espWeaponEnabled then
            local weapon = char:FindFirstChildOfClass("Tool")
            if weapon then
                data.WeaponLabel.Text = weapon.Name
        ...
        -- Flags (walking, jumping, swimming)
        if Yuno.espFlagsEnabled then
            local hum = char:FindFirstChildOfClass("Humanoid")
            if hum then
                local flags = {}
                ...
                data.FlagLabel.Text = table.concat(flags, " ")
                data.FlagLabel.Visible = #flags > 0
            else
...
local function getAimbotTarget()
    ...
    local targetType = Yuno.aimbotTarget
    ...
    for _, player in ipairs(Players:GetPlayers()) do
        ...
        local match = false
        if targetType == "Murderer" and role == "Murderer" then
            match = true
        elseif targetType == "Sheriff" and role == "Sheriff" then
            match = true
        elseif targetType == "Innocent" and role == "Innocent" then
            match = true
        elseif targetType == "Player" and (player.Name:lower():find(targetName:lower()) or player.DisplayName:lower():find(targetName:lower())) then
            match = true
        end
...
-- ============================================================
-- KILL ALL (Murderer) - with auto teleport
-- ============================================================
...
task.spawn(function()
    while RunService.RenderStepped:Wait() do
        ...
        if UserInputService.TouchEnabled then continue end -- handled by joystick
        local root = getHumanoidRootPart()
...
-- ============================================================
-- INSTANT ROUND WIN (teleport to finish)
-- ============================================================
...
-- ============================================================
-- UI CONSTRUCTION (WindUI) - ENHANCED
-- ============================================================
...
-- ===== ESP TAB (enhanced with box type, glow, filled, weapon, flags) =====
...
ESPTab:Dropdown({
    ...
    Values = { "2D", "Corner" },
    Value = Yuno.boxType,
    Callback = function(v) Yuno.boxType = v end
})
ESPTab:Toggle({
    Flag = "ESP_BoxGlow",
    Title = "Box Glow",
    Default = Yuno.boxGlowEnabled,
    Callback = function(v) Yuno.boxGlowEnabled = v end
})
ESPTab:Toggle({
    Flag = "ESP_BoxFilled",
    Title = "Filled Box",
    Default = Yuno.boxFilledEnabled,
    Callback = function(v) Yuno.boxFilledEnabled = v end
})
ESPTab:Toggle({
    Flag = "ESP_Name",
    Title = "Show Names",
...
ESPTab:Dropdown({
    ...
    Value = Yuno.lineOrigin,
    Callback = function(v) Yuno.lineOrigin = v end
})
ESPTab:Dropdown({
    Flag = "ESP_Filter",
    Title = "Player Filter",
...
ESPTab:Toggle({
    Flag = "ESP_Weapon",
    Title = "Show Weapon",
    Default = Yuno.espWeaponEnabled,
    Callback = function(v) Yuno.espWeaponEnabled = v end
})
ESPTab:Toggle({
    Flag = "ESP_Flags",
    Title = "Show Movement Flags",
    Default = Yuno.espFlagsEnabled,
    Callback = function(v) Yuno.espFlagsEnabled = v end
})
ESPTab:Toggle({
    Flag = "ESP_Gun",
    Title = "Gun ESP (highlight dropped gun)",
    Default = Yuno.espGunHighlight,
    Callback = function(v) Yuno.espGunHighlight = v end
})
...
-- ===== COMBAT TAB (enhanced aimbot target selection) =====
...
CombatTab:Dropdown({
    Flag = "AimbotTarget",
    Title = "Target Role / Player",
    Values = {
        {Title = "Murderer", Icon = "skull"},
        {Title = "Sheriff", Icon = "shield"},
        {Title = "Innocent", Icon = "user"},
        {Title = "Player (by name)", Icon = "user"},
    },
    Value = Yuno.aimbotTarget,
    Callback = function(selected) Yuno.aimbotTarget = selected.Title end
})
CombatTab:Input({
    Flag = "AimbotPlayerName",
    Title = "Player Name (if target is Player)",
    Placeholder = "Enter name",
    Value = Yuno.aimbotPlayerName,
    Callback = function(v) Yuno.aimbotPlayerName = v end
})
CombatTab:Slider({
    Flag = "AimbotSmooth",
    Title = "Smoothness",
...
CombatTab:Toggle({
    Flag = "AutoShoot",
    Title = "Auto Shoot Murderer (Headshot)",
...
-- ===== FLING TAB (enhanced with input, fling all, buttons) =====
...
FlingTab:Input({
    ...
    Placeholder = "Enter name",
    Value = Yuno.flingTargetName,
    Callback = function(v) Yuno.flingTargetName = v end
})
FlingTab:Button({
    Title = "Fling Target",
    Icon = "zap",
...
FlingTab:Toggle({
    ...
    Default = Yuno.flingAllEnabled,
    Callback = function(v)
        Yuno.flingAllEnabled = v
        if v then flingAll() end
    end
})
FlingTab:Button({
...
FlingTab:Button({
    Title = "Fling Sheriff (now)",
    Icon = "shield",
    Callback = function() flingSheriff() end
})

-- ===== MISC TAB (added FPS counter, rejoin already in teleport) =====
...
MiscTab:Toggle({
    ...
    Default = Yuno.fpsCounterEnabled,
    Callback = function(v)
        Yuno.fpsCounterEnabled = v
        if v then createFPSCounter() elseif fpsLabel then fpsLabel:Destroy(); fpsLabel = nil end
    end
})
MiscTab:Toggle({
    Flag = "TradeScam",
...
InfoTab:Section({ Title = "All-in-one MM2 Hub with all features from Benjo, Onyyx, Dabl Gee, Fan Hub, and more.", TextSize = 16, TextTransparency = 0.3 })
InfoTab:Section({ Title = "Features: ESP (Full customization) | Xray | Full Bright | Skybox | Hitboxes | Aimbot (target selection) | Auto Shoot (Headshot) | Kill All | Kill Aura | Infinite Ammo | Instant Reload | Auto Farm | Fly | Noclip | Infinite Jump | Bunny Hop | Speed Hack | Speed Glitch | Jump Power | Spinbot | Swim Walk | Teleport to Gun | Grab Gun | Weapon Spawn/Dupe/Replacer | Skin Changer | Fling (any player, all) | Trade Scam | Server Lagger | Anti-AFK | Bullet Tracers | FPS Counter | Rejoin | Config System", TextSize = 14, TextTransparency = 0.4 })
...
-- ============================================================
-- INITIALIZE XRAY, FULLBRIGHT, SKYBOX, FPS COUNTER, etc.
-- ============================================================
...
print("Yuno Hub loaded with all features including enhanced ESP, aimbot target selection, fling any player, FPS counter, and rejoin.")